      ...
      <div class="row">
        <div class="col-12 col-md-6">
          <div class="app--cutout-square">
            <h5 class="text text-success">Enter the length of cutout square (0 to {{maxSquareSize}})</h5>
            ...
            <div
              ...
              v-bind:style="{visibility: isInvalidInput ? 'visible' : 'hidden' }"
            >{{ errorMsg }}</div>
...
export default {
  ...
  methods: {
    ...
    handleCutout() {
      ...
      let biggestVolume = 0;
      for (let i = 0; i < this.boxVolumeArr.length; i++) {
    ...
    drawCanvas() {
      ...
      const unit = Math.min(
        Math.floor((canvas.width - 20) / this.paperWidth),
        Math.floor((canvas.height - 20) / this.paperLength)
      );
      drawPaper(
        canvas,
        this.paperWidth,
  ...
  computed: {
    volume: function() {
      return accMul(
        accMul(this.boxLength, this.boxWidth),
        this.boxSquareLength
      );
    }
  },
  created() {
