  ...
  <div class="container mt-4 mb-5">
    ...
    <div class="row">
      <div class="col-12 col-md-8 mb-4">
        <div class="app--ice-cream">
          <div class="app--ice-cream-flavours">
            <div class="app--img-flavours">
              <div class="mb-3" v-for="(flavour, index) in flavoursAvailable" :key="index">
                ...
                <img
                  ...
                  :style="{visibility: flavour.display ? 'visible' :'hidden' }"
                />
              </div>
            ...
            <div class="app-img-cup">
              ...
              >
                <img class="mr-1" :src="flavour.imgSrc" />
                <span class="badge badge-danger">{{ flavour.imgIndex }}</span>
              </div>
          ...
          <div class="app--ice-cream-combination">
            <div class="text text-danger" v-if="iceCreamExist">This ice cream has already been made</div>
            <div class="app--ice-cream-combination-record">
              <p>
                <span class="badge badge-danger">{{combinationsList.length}}</span>&nbsp;recorded
              </p>
              <p
                class="text text-success"
                v-if="iceCreamAllFound"
              >All ice creams are found. Please explain.</p>
            </div>
        ...
        <div class="app--action mt-3 mb-3">
          <button
            ...
            @click="handleNext"
          >Show next ice cream</button>
          <button
            class="btn btn-outline-success btn-lg mb-3"
            v-if="demoAutoOption==='1'"
            @click="handleToggleTimer"
          >{{ timer ? "Pause" : "Start" }}</button>
          <div>
      ...
      <div class="col-12 col-md-4 app--ice-cream-number">
        ...
        <div v-for="(num, index) in numberOfPossibleFlavours" :key="index" class="form-group row">
        ...
        <div class="row mt-3">
          <h6 class="text text-success">Calculate the number of possible ice creams</h6>
          <div class="col-10 col-md-8">
            <input
              type="number"
              class="form-control"
...
export default {
  ...
  methods: {
    ...
    setNumberOfPossibleFlavours() {
      // Calculate the number of possibility for each flavour
      this.numberOfPossibleFlavours.splice(
        0,
        this.numberOfPossibleFlavours.length
      );
    ...
    setTotalNumberOfIceCream() {
      // Calculate the total number of possibilities
      let totalNumber = 1;
      if (
        this.numberOfPossibleFlavours ||
        this.numberOfPossibleFlavours.length === 0
      ) {
    ...
    setNumberOfPossibleFlavoursInputCheck(index = -1) {
      // Check users input
      this.numberOfPossibleFlavoursInputCheck.splice(
        0,
        this.numberOfPossibleFlavoursInputCheck.length
      );
      for (let i = 0; i < this.numberOfPossibleFlavours.length; i++) {
    ...
    getCurrentCombination(flavoursSelected) {
    ...
    setCombinationsList(flavoursSelected) {
      // Add a combination to combinations list
    ...
    handleSelectFlavour(e) {
      ...
      if (!this.allowRepeat) {
        ...
        // });
        this.flavoursAvailable.forEach(flavour => {
          if (flavour.imgIndex === imgIndex) {
            flavour.display = false;
          }
        });
      }
      ...
      if (
        this.flavoursAvailable.length === 0 ||
        this.flavoursSelected.length === Number(this.numberOfScoops)
      ) {
        this.foundOne = true;
        const combination = this.getCurrentCombination(this.flavoursSelected);
        if (this.combinationsList.indexOf(combination) !== -1) {
    ...
    setNextFlavours() {
      ...
      do {
        ...
        nextFlavoursSelected &&
        this.combinationsList.indexOf(nextCombination) !== -1
      );
