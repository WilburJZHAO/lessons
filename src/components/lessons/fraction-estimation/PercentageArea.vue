    ...
    <div class="row">
      ...
      <div class="col-md-9">
        <app-strip
          ...
          @estimateAccuracy="handleEstimateStripAccuracy"
          v-if="graphType===1"
        ></app-strip>
        <app-tower
          :status="status"
          :percentageData="percentageData"
          @updateStatus="status = $event"
          @estimateAccuracy="handleEstimateTowerAccuracy"
          v-if="graphType===2"
        ></app-tower>
        <app-pie
          :status="status"
          :percentageData="percentageData"
          @updateStatus="status=$event"
          @estimateAccuracy="handleEstimatePieAccuracy"
          v-if="graphType===3"
        ></app-pie>
        <div class="text-danger mb-3">
          <span
            v-if="status === 0 || status === 1 || status === 3"
          >Click in the yellow rectangle to estimate the given percentage.</span>
...
export default {
  ...
  methods: {
    ...
    handleEstimateStripAccuracy($event) {
      this.handleEstimateAccuracy($event);
    },
    handleEstimateTowerAccuracy($event) {
      this.handleEstimateAccuracy($event);
    },
    handleEstimatePieAccuracy($event) {
      this.handleEstimateAccuracy($event);
    },
    handleEstimateAccuracy(accuracy) {
      this.estimateAccuracy = accuracy;
